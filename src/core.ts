...
try {
  const storedUserId = localStorage.getItem("unitracker-user-id");
  if (storedUserId) {
    customUserId = storedUserId;
  }

  const optOutStatus = localStorage.getItem("disable-unitracker");
...
if (typeof window !== "undefined" && (window as any).__UNITRACKER_OPTOUT__) {
  isOptedOut = true;
}
...
export function identify(userId: string): void {
  ...
  try {
    localStorage.setItem("unitracker-user-id", customUserId);
...
export function clearUserId(): void {
  customUserId = null;
  try {
    localStorage.removeItem("unitracker-user-id");
    log("User ID cleared");
  } catch (e) {
