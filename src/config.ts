import { InternalUniTrackerConfig,UniTrackerConfig } from "./types";
import { log, logError } from "./utils";

// Local-only defaults (not controlled by remote config)
const localDefaults: Required<Omit<UniTrackerConfig, "analyticsHost" | "siteId" | "replayPrivacyConfig">> =
  {
    debounce: 500,
    skipPatterns: [],
    maskPatterns: [],
...
let internalConfig: InternalUniTrackerConfig | null = null;

export const currentConfig: Readonly<InternalUniTrackerConfig> = new Proxy(
  {} as InternalUniTrackerConfig,
  {
    get: (_, prop: keyof InternalUniTrackerConfig) => {
      if (!internalConfig) {
        if (prop !== "debug") {
          logError(
            "UnifyTracker SDK accessed before initialization. Call uniTracker.init() first."
          );
        }
    ...
    set: () => {
      logError("UnifyTracker config is read-only after initialization.");
      return false;
    },
  }
);
...
export async function initializeConfig(options: UniTrackerConfig): Promise<boolean> {
  if (internalConfig) {
    logError("UnifyTracker SDK already initialized.");
    return false;
  }

  if (typeof options !== "object" || options === null) {
    logError(
      "Invalid configuration provided to uniTracker.init(). Expected an object."
    );
    return false;
  }
  ...
  if (!analyticsHost || analyticsHost.trim() === "") {
    logError(
      "`analyticsHost` is required in UnifyTracker config and must be a non-empty string."
    );
    return false;
  }
